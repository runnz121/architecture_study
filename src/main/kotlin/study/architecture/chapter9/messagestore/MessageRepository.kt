...
@Repository("ch9MessageRepository")
interface MessageRepository : JpaRepository<Message, UUID> {

    @Query("SELECT MAX(m.position) FROM Ch9Message m WHERE m.streamName = :streamName")
    fun findMaxPositionByStreamName(@Param("streamName") streamName: String): Long?

    @Query("SELECT MAX(m.globalPosition) FROM Ch9Message m")
    fun findMaxGlobalPosition(): Long?

    @Query(
        """
        SELECT m FROM Ch9Message m
